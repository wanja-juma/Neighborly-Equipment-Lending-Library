...
@profile_bp.get("/profiles/<int:profile_id>")
@jwt_required()
def get_profile(profile_id):
    ...
    return jsonify({"profile": profile_schema.dump(profile)}), 200
...
class OwnProfile(Resource):
    ...
    def put(self):
        ...
        try:
            ...
            return {"error": "Invalid authentication identity."}, 401

        profile = db.session.scalar(
            db.select(Profile).where(Profile.user_id == current_user_id)
        )
        ...
        if attempted_protected_fields:
            return {
                "error": "The id and user_id fields cannot be updated."
            }, 400
        ...
        try:
            ...
            return {
                "error": "Unable to update the profile due to a data conflict."
            }, 409

        return {
            "message": "Profile updated successfully.",
            "profile": profile_schema.dump(updated_profile),
        }, 200
