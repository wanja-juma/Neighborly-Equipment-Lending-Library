...
class BorrowingRequestSchema(SQLAlchemyAutoSchema):
    ...
    # Extra frontend-friendly computed fields
    ...
    def get_item(self, borrowing_request):
        item = getattr(borrowing_request, "item", None)
        if item is None:
            return None
    ...
    def get_owner_id(self, borrowing_request):
        item = getattr(borrowing_request, "item", None)
        if item is None:
            return None
    ...
    def get_borrower(self, borrowing_request):
        user = getattr(borrowing_request, "user", None)
        if user is None:
            return None
