...
const USER_STORAGE_KEY = "neighborlyUser";
const TOKEN_STORAGE_KEY = "neighborlyToken";

const clearStoredAuthentication = () => {
  localStorage.removeItem(USER_STORAGE_KEY);
  localStorage.removeItem(TOKEN_STORAGE_KEY);
  localStorage.removeItem("neighborlyRefreshToken");
...
const normalizeUser = (user) => {
  ...
  return {
    ...
    firstName,
    lastName,
    name: user.name || fullName || user.email || "Neighbour",
...
function AuthProvider({ children }) {
  ...
  const login = useCallback(
    (user, token) => {
      ...
      if (!token) {
        throw new Error("An access token was not returned by the server.");
      }
  ...
  useEffect(() => {
    const restoreSession = async () => {
      ...
      try {
        ...
        const restoredUser = responseBody?.user;
